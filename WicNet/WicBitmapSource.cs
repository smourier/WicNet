...
public sealed class WicBitmapSource : IDisposable, IComparable, IComparable<WicBitmapSource>
{
    ...
    public WicBitmapSource(object comObject) => _comObject = new ComObjectWrapper<IWICBitmapSource>(comObject).ComObject;
    ...
    public WicIntSize Size => new(Width, Height);
    public WICRect Bounds => new(0, 0, Width, Height);
    ...
    {
        var bmp = _comObject.As<IWICBitmap>() ?? throw new WicNetException("WIC0004: Converting to WinRT SoftwareBitmap is only supported on in-memory bitmaps. You must Clone this bitmap first.");
    ...
    {
        var bmp = _comObject.As<IWICBitmap>() ?? throw new WicNetException("WIC0002: Lock is only supported on in-memory bitmaps. You must Clone this bitmap first.");
        return bmp;
    }
