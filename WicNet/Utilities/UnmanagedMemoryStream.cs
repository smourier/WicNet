...
public sealed class UnmanagedMemoryStream : Stream, IStream
{
    ...
    public IStream NativeStream
    {
        get
        {
            var stream = _stream ?? throw new ObjectDisposedException(nameof(NativeStream));
            return stream;
        }
    }
    ...
    void IStream.Stat(out STATSTG pstatstg, int grfStatFlag) => NativeStream.Stat(out pstatstg, grfStatFlag);
